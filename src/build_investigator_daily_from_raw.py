...
def date_horizon(
    typed: pd.DataFrame, pad_days: int = 14
...
def build_wip_series(
    ...
    if not deltas:
        ...
        return pd.DataFrame({"date": idx, "staff_id": [], "team": [], "wip": []}).head(
            0
        )
...
def build_daily_panel(
    ...
    grid = (
        grid.merge(backlog, on="date", how="left")
        .fillna({"backlog_available": 0})
        .sort_values(["staff_id", "date"])
    )
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Build per‑investigator daily panel from OPG investigations extract."
    )
