...
This notebook and documentation were prepared for the OPG forecasting workstream by Leila Yousefi. If you are taking over, start by running a short date window end to end, inspect the output tables, and then expand the window once you are confident the definitions match the reporting need.

#==============================================================================
## @author: Dr. Leila Yousefi 
## MoJ Modelling Hub - OPG Forecasting Team
#==============================================================================
&nbsp;
&nbsp;
...
&nbsp;
# [Model](#modeldoc) - Model Documentation: Deputyship Data Analysis and Modelling
&nbsp; 
# [Inputs](#inputs)
&nbsp;
# [Outputs](#outputs)
...
- To forecast income from cases under active supervision, changes in the age structure of supervised cases and impact on remissions and exemptions; necessary to forecast investigations arising from deputyships. Long term projections (5 years +). Annual numbers of new deputyship cases, and numbers of cases under active supervision. Frequency: annual updates.
...
&nbsp;
<a name="modeldoc"></a>
# Model Documentation: Deputyship Data Analysis and Modelling

## 1. Introduction
This document outlines the key insights, data requirements, and modelling strategies discussed regarding the analysis of deputyship data from the Court of Protection. The primary goal is to refine forecasting models for active deputyship caseloads, with a focus on simplicity, accuracy, and policy implications.
...
## 2. Overview of Deputyship Data

### 2.1 Data Components:
- **Order Number**: Unique identifier used by the Court of Protection.
- **Case Number**: Tracks individual cases and helps manage multiple records for the same person. 
    A unique identifier for one person and should be for a specific individual (same dob and postcode).
- **Order Status**: Identifies whether a case is Active or Closed. By having Active and Closed type we can calculate the time of placing an order (Active) and 
    closing that order (by looking at the Closed cross refrencing with the termination date). Also findout the age of the donor when case was opened (Active) 
    and when it was terminated (Closed). 
    *If the status in Closed and the termination date was empty then neccessarily it does not mean that the order was terminated.
- **Termination Date**: Marks the end of an order due to various reasons (e.g., death of the protected person or deputy).
- **Historical Data**: Provides insights into initial order dates, replacements, and trends.
- **Ord Risk Lvl**: (Order Risk Level) a risk assessment level of deputyships under active supervision regarding the the application finantial situation (1:minimum, 2 and 3:general free).
- **Create**: A date of order was created.
- **Order No**: A ref that the Court of Protection (CoP) is using.
- **Order Type**: A number categorised based on the Desc.
- **AppNo**: The number of orders for an individual and so the corrsponding case number.
- **Desc**: (Order Description) Property & Affairs Deputy (Financial) / Replacement P&A Order / Interim P&A Order / Health & Welfare Deputy.
- **Made Date**: A date of order was placed.

### 2.2 Key Variables:
- **Age of Protected Person**: Used to estimate termination probabilities.
- **Order Risk Level**: Categorises cases into general or minimal fee payers.
- **Financial Assessment Data**: Determines fees based on liquid assets (threshold: £30,000) and look at the household income in family resource survey if they are in the receive of benefit or own house to decide the remission and exemption.

---

## 3. Modelling Deputyship Caseload

### 3.1 Current Modelling Challenges/Risks:
- **Cohort-Based Survival Rates**: The existing approach tracks individuals from the start of their deputyship, but this method is complex and prone to inaccuracies due to limited data.
- **Historical Gaps**: Delay in recording and outdated datasets (CASRAC) hinder accurate trend analysis. And you cannot be confidenet how many orders was made in a year.

### 3.2 Proposed Simplification:
- Use **age-specific termination rates** instead of cohort-based survival rates.
- Treat active caseloads as a "stop-flow" model:
  - The Active caseloads are the water in a bath and the water flowing into the bath are the new deputyship orders. The water flowing out of bath are the terminated cases.
  - **Inflow**: New deputyship orders.
  - **Outflow**: Cases terminated due to death or other reasons.
- Apply a simple proportion for annual termination rates to update the active caseload. 
- We need to finout what is the flow of water going into the bath (the population at the risk having a deputyship and how this rate changes) in next five years.
...
### 4.1 Current Data:
- Existing data from the legacy CASRAC system includes partial information.
- Requires updated data from the Sirius platform for more comprehensive analysis.

### 4.2 Additional Data Needs:
- **Fee Data**: Details of exemptions, remissions, and thresholds.
- **Demographics**: Breakdown of deputyship cases by age, risk level, and financial status.
- **Trend Data**: Annual figures for new deputyships and termination rates.

### 4.3 Integration Needs:
- Synchronise deputyship data with Power of Attorney (LPA) models to understand overlapping trends.
- Ensure compatibility with government datasets for broader demographic and policy analysis.
...
### 5.1 Fee Structures:
- General fee (£360/year) vs. minimal fee (£32/year).
- Historical debates on equalising fees due to perceived inequities.

### 5.2 Financial Modelling:
- Revenue forecasting for the OPG based on changes in caseload and fee structures.
- Impact of potential policy changes, such as equalising fees or adjusting thresholds.

### 5.3 Broader Applications:
- Use of the model to predict resource needs and budget allocations.
- Insights into the impact of increasing uptake of LPAs on deputyship caseloads.
...
1. Obtain and preprocess the latest deputyship data from Sirius.
2. Validate data integrity and identify gaps in key variables.
3. Implement simplified termination rate calculations in the model.

### 6.2 Long-Term Goals:
1. Expand the model to differentiate between fee categories, remissions, and exemptions.
2. Use micro-simulation techniques to enhance policy scenario analysis.
3. Regularly update the dataset and recalibrate the model for accuracy.

---

## 7. Conclusion
This documentation serves as a framework for analysing and modelling deputyship data. By simplifying the model and incorporating updated datasets, the analysis can offer more accurate forecasts and support critical policy decisions. The outlined steps aim to streamline the process and provide actionable insights for the OPG and related stakeholders.
To determine the active caseload each year, we need to calculate how many new cases are added and how many cases are terminated. These are the two key factors.
Currently, we don't have records that correspond directly to the order of the statistics to the termination dates. Not all closed cases have termination dates. This is because some cases are closed due to amendments to the order rather than termination of the deputyship. For example, a replacement order might be made, and the previous order is closed, but the deputyship remains active.
To simplify the model, we can use a straightforward method:
1. **Add New Cases**: Each year, add the number of new deputyships to the active caseload. ALL UNIQUE CASES (ACTIVE, OPEN, CLOSED) BY YEAR AND AGE CREATED
2. **Apply Termination Rates**: Assume a certain proportion of the active caseload will terminate each year, based on termination rates. Applying a percentage-based termination rate to the active caseload.
For example, if we add 50 new cases aged 29 to the active caseload and assume a 1% termination rate, we expect 1% of those cases to terminate by the following year. This process is repeated each year.
This method is simpler than the current cohort-based model, which tracks survival rates over time. The simpler method involves applying a crude termination rate to each age group, making it easier to manage and potentially more accurate.
...
1. The deputyships orders are made for each individual cases by the Court of Protection from CASRAC and Sirius databases
    - Thanks to the data engineering team (Philip Hall) have now put together a first draft of the analytical endpoint table based on our advice by using create-a-derived-table. The table is at guardianship_derived_dev_dbt.analytical_endpoint. 
    - This has provided a range of columns for the different actors to show the different ways that the data can be assembled from the data model. So for donors, have included all of the geographical columns from the NSPL, rather than the donor address itself. 
    - For Certificate Providers, have included actual address details. In either case, these can be modified.
    - There is a need to analyse the underlying design in more detail and explore how we are intending to use this and the best way to provide the data for the OPG bussiness purposes.
...
6. Number of LPA application (to be suntracted for the polulation to provide number of deputyships)
...
*under constuction!
Process Map: Deputyship Data Analysis and Modelling

1. Data Collection
Identify required data points (e.g., order status, termination date, historical trends).
Gather current data from legacy systems (CASRAC) and new systems (Sirius).
Validate the completeness and integrity of the data.
2. Data Preprocessing
Filter data to focus on active cases for immediate analysis.
Deduplicate records using case numbers and track historical trends.
Extract relevant variables like age, order risk level, and financial data.
3. Modelling
Current Approach: Use cohort-based survival rates (complex and detailed).
Proposed Simplification:
Treat active caseloads as a stop-flow model:
Inflow: New deputyship orders.
Outflow: Cases terminated (e.g., death, order closure).
Apply age-specific termination rates to forecast changes in active caseload.
Incorporate insights from Power of Attorney (LPA) models to cross-reference trends.
4. Policy and Financial Analysis
Evaluate current fee structures (e.g., general vs. minimal fees).
Assess the impact of policy changes on revenue (e.g., fee equalization).
Use projections to inform policy decisions, including fee remission and exemptions.
5. Scenario Testing and Enhancements
Test different termination rate assumptions to refine forecasts.
Decompose caseloads into subcategories (e.g., fee payers vs. exemptions).
Explore micro-simulation techniques for detailed financial analysis.
6. Future Data Integration
Synchronize deputyship data with LPA datasets to identify overlaps and gaps.
Collaborate with stakeholders to access and curate comprehensive datasets.
Regularly update datasets and recalibrate the model based on new data.
7. Outputs
Produce actionable insights for:
Forecasting active caseload trends.
Projecting revenue impacts of policy changes.
Informing resource allocation for OPG and related stakeholders.
...
It is exactly analogous to basically a bath that is being filled up with water with a tap, and water is flowing out of the bath at the same time. So what we have got is the active caseload, which is the water in the bath. The water that is flowing into the bath is the new deputyships. So these are the new cases that are being added to the active case load. So that is why we need to know to understand the trend in the active caseload is. So 
...
2. Order No: a ref that the Court of Protection (CoP) is using.

3. Case: a unique identifier for one person and should be for a specific individual (same dob and postcode).

4. AppNo: number of orders for an individual and so the corrsponding case number.

5. Ord Stat: Active/Closed

6. Ord Type: 

7. Desc: Order Description: Property & Affairs Deputy (Financial) / Replacement P&A Order / Interim P&A Order / Health & Welfare Deputy
...
guardianship_derived_dev_dbt.analytical_endpoint
...
>> dbt seed 
>> dbt seed --select "...__..."  
>> dbt seed --select "opg__opg_sirius_lookups"
>> dbt run --select <-Paths to the corresponding directory->
>> dbt run --select models/general/fam_derived_v4_db/base/
```
&nbsp;
<a name="3how2"></a> 
