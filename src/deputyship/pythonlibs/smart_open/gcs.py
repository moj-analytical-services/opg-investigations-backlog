...
class Reader(io.BufferedIOBase):
    ...
    def __init__(
        ...
        self._blob = client.bucket(bucket).get_blob(
            key
        )  # type: google.cloud.storage.Blob

        if self._blob is None:
            raise google.cloud.exceptions.NotFound(
                "blob %s not found in %s" % (key, bucket)
            )
...
class Writer(io.BufferedIOBase):
    ...
    def __init__(
        ...
        assert (
            min_part_size % _REQUIRED_CHUNK_MULTIPLE == 0
        ), "min part size must be a multiple of 256KB"
        assert (
            min_part_size >= _MIN_MIN_PART_SIZE
        ), "min part size must be greater than 256KB"
