...
**OPG Data Dictionary
https://data-dictionary.opg.service.justice.gov.uk/
...
1. Run the first chunck (Step 1: Python Packages) in JypterLab to install packages.
2. Run the second chunck in JypterLab:
    1. Step 2.1: DataHandler Class
    2. Step 2.2: DataPreprocessor Class
    3. Step 2.3: Create Objects and instance of the classes
    4. Step 2.4: Handelling Missing Data
    5. Step 2.5: Data Manipulation
3. Download "LPA_DATA.csv" CSV file from the File Browser in JypterLab.
4. Copy the LPA_DATA" CSV file in "LPA_DATA" tab in the last version of the LPA excel model (e.g., "OPG_DemandForecasts_Jan25_v3Peter (version Period Mortality)_L").
5. Run the third chunck for the short-term forecasting of the current year:
    1. Step 3.1: Short-term Forecasting of the Current year - Scenario 1
    2. Step 3.2: Short-term Forecasting of the Current year - Scenario 2
    3. Step 3.3: Short-term Forecasting of the Current year - Scenario 3
    4. Step 3.4: Short-term Forecasting of the Current year - Selection of a Scenario
    5. Step 3.5: Short-term Forecasting of the Current year - The final Donor Estimates & Actuals
6. Run the fourth chunk to generate Survival Tables:
...
3. Trends and patterns of the LPA orders
4. Data modelling (Dimentional Modelling in CaDT)
...
![Survival Analysis Tools](../images/Survival_analysis_tools.jpg)
*ref: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6110618/
...
#### Assumptions and Risks in Mortality Statistics
...
- To simplify things and given the very small difference in mortality rates between England and Wales for 2017-19 that we could just use the mortality rates for England. Once we have more time we can add some complexity by adding variants such as rates for Wales or combined England and Wales estimates. 
...
*e.g., if there are 1000 people and 100 of them are still alive and bought LPA, so there are 900 of them still didn't buy LPA.
...
For each year calculate the sum/total of (previous year in each group group: [18-20, 21-25, 26-30, 31-35, 36-40, 41-45, 46-50, 51-55, 56-60, 61-65, 66-70, 71-75, 76-80, 81-85, 86-90, 91-95, 96-100, 101+])


### Analysis of **Demographic Ageing** LPA SURVIVAL TABLES (BY YEAR OF APPLICATION): 
Build a model to calculate Survival rate based on **demographic ageing** for each year (from 2007 to 2030) and age (from 18 to 106) based on scikit-survival or any other survival package in python. The inputs is the exisiting mortality rates, that is the probability that a person aged x exact will die before reaching age (x +1) and 1), as an array of probabilities of qx, where qx is the mortality rate between age x and (x +1). The mortality rates allow comparisons between populations with different age structures, including between males and females and over time and represent the proportion of deaths per 1000 live births. 

1. Create emographic ageing cohort model by generating an individual table for each year from year = 2007 to 2023 and age = 18 to 106. E.g., for year= 2017 generate a LPA SURVIVAL TABLE table named 'LPA SURVIVAL TABLE 2007' and the values in 'LPA SURVIVAL TABLE 2007' corresponded to that year (e.g., 2017 and age = 18) should be filled by using the number of donor demands ('demand' column) for that corresponding age and year calculated in "annual_unique_donors (2)" sheet. 

2. Calculate 'ESTIMATED AGE SPECIFIC SURVIVAL' table is calculate using the mortality rate (qx) in the 'Mortality Statistic' sheet. 
  
3. For the follwoing years after the corresponding year, we would need to use the 'Mortality Statistics': MALE/FEMALE: ESTIMATED AGE SPECIFIC SURVIVAL tables and calculate the number of donors based on the survival rate:
Where prior_number_of_year = the corresponding year REDUCTED from the starting (base) year 2007 = the number of previous years till the corresponding year to track back and calculate the 'ESTIMATE AGE SPECIFIC SURVIVAL' for male/female.
IF: 'LPA Control Assumptions': FORECAST ASSUMPTIONS: MORTALITY RATES = 1 (Male) 
THEN: ((the number of donors for **prior_number_of_year** years before the corresponding year (age - **prior_number_of_year**))
MULTIPLIED BY ('Mortality Statistics': MALE: ESTIMATED AGE SPECIFIC SURVIVAL) for the corresponding age and +**prior_number_of_year**)
OTHERWISE: ((the number of donors for **prior_number_of_year** years before the corresponding year (age - **prior_number_of_year**))   
MULTIPLIED BY ('Mortality Statistics': FEMALE: ESTIMATED AGE SPECIFIC SURVIVAL) for the corresponding age and +**prior_number_of_year**)

For example: the next year column in 'LPA SURVIVAL TABLE 2007' (e.g., year = 2018) fill the values by multyiplying the previous year (e.g., 2017) value for that specific age from column number = this year - previous year and corresponded row in 'ESTIMATED AGE SPECIFIC SURVIVAL'. 
For the next  

4. For each corresponding year and age group (Where year = 2007 to 2023 and age = 18 to 106) do the steps above.

5. For (Where year = 2024 to 2030 and age = 18 to 106) use frecasted value for the number of donors (Final Forecast (Mean) in "LongTermForecastMarket" sheet).

6. Feed into the "LongTermForecastMarket" sheet by generating a new forecasted values in another column based on the survival rate in previous years

7. For each year and age specific value in a new sheet "Total LPA SURVIVAL TABLES" that generated by adding up all of values for each year and each age from all LPA SURVIVAL TABLEs from 2017 up to that year.
