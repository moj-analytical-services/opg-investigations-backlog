...
def fit_legal_review_enet(df: pd.DataFrame, y_name: str = "needs_legal_review", random_state: int = 42):
    ...
    pre = build_preprocessor(df, y_name=y_name)                 # impute, scale, OHE, TE (if high-card present)
    clf = LogisticRegressionCV(                                 # CV over C and l1_ratio for elastic-net
        penalty="elasticnet", solver="saga",
        l1_ratios=[0.1, 0.5, 0.9], Cs=20,
        cv=StratifiedKFold(5, shuffle=True, random_state=random_state),
        scoring="roc_auc", max_iter=5000, n_jobs=-1, class_weight="balanced"
    )
    ...
    metrics = {"auc": float(auc), "average_precision": float(ap), "brier": float(brier),
               "calibration_frac_pos": frac_pos, "calibration_mean_pred": mean_pred}
    return pipe, metrics
...
def fit_backlog_glm(daily_backlog: pd.DataFrame, staffing: pd.DataFrame):
    ...
    model = poisson if overdisp <= 1.0 else sm.GLM(y, X, family=sm.families.NegativeBinomial()).fit()
    return model, df
...
def fit_survival_pg(df: pd.DataFrame):
    ...
    sdf = df[["days_to_pg_signoff", "event_pg_signoff", "case_type", "risk", "weighting"]].dropna(subset=["days_to_pg_signoff"]).copy()
