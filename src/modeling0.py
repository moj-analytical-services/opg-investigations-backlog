...
def forecast_backlog(model, df_template: pd.DataFrame, days: int = 90):
    ...
    base_t0 = (
        pd.to_datetime(df_template["date"]) - pd.to_datetime(df_template["date"]).min()
    ).dt.days.max()
    Xf = pd.DataFrame(
        {
            "const": 1.0,
...
def fit_legal_review_classifier(df: pd.DataFrame):
    ...
    pipe = Pipeline(
        [("pre", pre), ("clf", LogisticRegression(max_iter=200, solver="liblinear"))]
    )
...
def fit_survival_model(df: pd.DataFrame):
    sdf = df[
        ["days_to_pg_signoff", "event_pg_signoff", "case_type", "risk", "weighting"]
...
def scenario_apply_staffing(df_template: pd.DataFrame, delta_investigators: int, model):
    ...
    df_scn["investigators_on_duty"] = np.maximum(
        0, df_scn["investigators_on_duty"] + delta_investigators
    )
