...
def basic_clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "weighting" in df.columns:
        df["weighting"] = (
            pd.to_numeric(df["weighting"], errors="coerce").fillna(0).clip(0, 5)
        )
    return df
...
def engineer_intervals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"pg_signoff_date", "date_received_opg"} <= set(df.columns):
        df["days_to_pg_signoff"] = (
            df["pg_signoff_date"] - df["date_received_opg"]
        ).dt.days
    else:
    ...
    if "status" in df.columns:
        df["is_backlog"] = (
            df["status"]
            .str.lower()
            .isin(["awaiting_investigator", "to_be_allocated"])
            .astype(int)
        )
    else:
        df["is_backlog"] = 0
    ...
    has_any_legal = (
        df[has_legal_cols].notna().any(axis=1)
        if has_legal_cols
...
def aggregate_staffing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = (
        out.groupby(pd.to_datetime(out["date"]))
        .agg(
...
def generate_synthetic(n: int = 8000, seed: int = 42) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        {
            ...
            "weighting": rng.choice(
                [0, 1, 2, 3, 4], size=n, p=[0.1, 0.25, 0.2, 0.4, 0.05]
            ),
            "risk": rng.choice(risks, size=n, p=[0.15, 0.45, 0.25, 0.05, 0.10]),
            "case_type": rng.choice(
                case_types, size=n, p=[0.6, 0.1, 0.05, 0.1, 0.1, 0.05]
            ),
            "status": rng.choice(
                status_states, size=n, p=[0.05, 0.25, 0.4, 0.2, 0.05, 0.05]
            ),
        }
    )
    start_offsets = rng.integers(0, 1200, size=n)
    df["date_received_opg"] = base_date + pd.to_timedelta(start_offsets, unit="D")
    alloc_delay = np.maximum(
        0, rng.normal(loc=25 - 2 * df["weighting"], scale=10, size=n)
    ).astype(int)
    df["date_allocated_investigator"] = df["date_received_opg"] + pd.to_timedelta(
        alloc_delay, unit="D"
    )
    ct_factor = (
        df["case_type"]
    ...
    risk_factor = (
        df["risk"]
        .map({"Very High": 20, "High": 15, "Medium": 10, "Low": 5, "Normal": 7})
        .fillna(7)
    )
    signoff_delay = np.maximum(
        0, rng.normal(loc=ct_factor + risk_factor, scale=10)
    ).astype(int)
    df["pg_signoff_date"] = df["date_allocated_investigator"] + pd.to_timedelta(
        signoff_delay, unit="D"
    )
    ...
    if needs_legal.sum() > 0:
        df.loc[needs_legal, "legal_approval_date"] = df.loc[
            needs_legal, "date_allocated_investigator"
        ] + pd.to_timedelta(approve_delay[: needs_legal.sum()], unit="D")
    return df
