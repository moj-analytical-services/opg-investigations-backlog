...
from .intervals import (
    build_event_log, build_wip_series, build_backlog_series,
    build_daily_panel, summarise_daily_panel
)
...
@click.option("--outdir", type=click.Path(file_okay=False), default="data/processed",
...
@click.option("--eng", "eng_csv", type=click.Path(exists=True, dir_okay=False), required=True,
...
def intervals_cmd(eng_csv: str, outdir: str):
    ...
    events = build_event_log(df); events_out = out_dir / "event_log.csv"; events.to_csv(events_out, index=False)

    # 2) Backlog/WIP/Daily panel (your existing notebook functions)
    backlog = build_backlog_series(df); backlog_out = out_dir / "backlog_series.csv"; backlog.to_csv(backlog_out, index=False)
    daily   = build_daily_panel(df);     daily_out   = out_dir / "daily_panel.csv";     daily.to_csv(daily_out, index=False)

    # 3) Summary table (unchanged logic)
    summary = summarise_daily_panel(df); summary_out = out_dir / "daily_panel_summary.csv"; summary.to_csv(summary_out, index=False)
...
@click.option("--backlog", "backlog_csv", type=click.Path(exists=True, dir_okay=False), required=False,
              help="Optional: backlog_series.csv if your IntervalAnalysis uses it.")
@click.option("--out", type=click.Path(dir_okay=False), default="reports/last_year_by_team.csv",
...
@click.option("--group", default="case_type", help="Grouping column (default: case_type).")
@click.option("--outdir", type=click.Path(file_okay=False), default="reports",
