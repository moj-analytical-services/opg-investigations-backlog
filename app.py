...
if st.button("Run simulation"):
        ...
        while True:
            ...
            metrics.append(
                dict(
                    day=day,
                    open=len(open_cases),
                    over30=sum(a > 30 for a in ages),
                    over60=sum(a > 60 for a in ages),
                    over120=sum(a > 120 for a in ages),
                )
            )
            yield env.timeout(5)
    ...
    st.subheader("Backlog size (open cases)")
    st.line_chart(kpi.set_index("day")["open"])
    st.subheader("Ageing profile")
    st.line_chart(kpi.set_index("day")[["over30", "over60", "over120"]])
    st.download_button(
        "Download KPI CSV", kpi.to_csv(index=False), "kpi.csv", "text/csv"
    )
