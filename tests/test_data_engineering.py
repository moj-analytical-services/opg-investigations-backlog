...
def test_date_horizon_basic(typed_df_small, horizon):
    ...
    assert (end - start).days >= (
        pd.Timestamp("2025-01-06") - pd.Timestamp("2025-01-01")
    ).days
...
def test_build_backlog_series_monotone(typed_df_small, horizon):
    ...
    assert (
        backlog["backlog_available"].dtype.kind in "iu"
        or backlog["backlog_available"].dtype.kind == "f"
    )
...
def test_build_daily_panel_contract(typed_df_small, horizon):
    ...
    if not events.empty:
        assert set(events["event"]).issubset(
            {"newcase", "legal_request", "legal_approval", "court_order"}
        )
